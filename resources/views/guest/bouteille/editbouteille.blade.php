    ...
    <form class="formulaire" action="{{ route('bouteilles.update', [$cellier->id, $bouteille->id]) }}" method="post">
        ...
        @else
            ...
            <div>
                <label for="pays">Pays</label>
                <select name="pays">
                    <option value="{{old('pays', $bouteille->pays)}}" selected="selected">{{old('pays', $bouteille->pays)}} </option>
            ...
            <div>
                ...
                <textarea name="description" id="description" cols="30" rows="10">{{old('description', $bouteille->description)}}</textarea>
                @error('description')
                    <div class="text-danger">{{ $message }}</div>
                @enderror
            </div>
